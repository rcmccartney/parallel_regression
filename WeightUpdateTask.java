 ...
 */
public class WeightUpdateTask extends Task {
			
	int K, size, steps, snap;
	double eps, alpha, lambda;
	double[] weights, weightUpdate;
	...
	public void main(String[] args) throws Exception {
		...
		WeightTuple tuple = new WeightTuple();
		WeightTuple wt = null;
		tuple.step = 0;
		...
		UpdateTuple ut = null;
		double log_error_sum, delta, sse, regularize;
		...
		for (;;) {
			...
			for (int i = 0; i < K; ++ i) {
				...
				for(int j=0; j < weightUpdate.length; j++)
					weightUpdate[j] += ut.weightUpdate[j];
			}
			...
			// cost + regularization 
			cost.add((log_error_sum + lambda*regularize/2)/size);  
			sqr_err.add(sse/size); // store avg squared error
			// update the weight vector using gradient descent
			//don't regularize the intercept term
			weights[0] -= (alpha/size)*weightUpdate[0];
			for(int j=1; j < weights.length; j++)
				weights[j] -= (alpha/size)*(weightUpdate[j]+lambda*weights[j]);
